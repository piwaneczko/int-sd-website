...
[`~/scripts/ollama-sanitize-changelog.sh`](../../scripts/ollama-sanitize-changelog.sh)
(run as part of each repo's own `scripts/release.sh`) via a local Ollama
model — same connection config as `ollama-commit.sh` /
`ollama-update-changelog.sh` (`~/scripts/lib/ollama-config.sh`:
...
- caches already-sanitized releases by a hash of the source entry
  (`.changelog-sanitize-cache.json`, committed next to `CHANGELOG.public.md`)
  so a published release's public text doesn't drift on a later run,
- validates the model's output against `~/scripts/lib/changelog-deny-list.txt`
  (technical terms, hardware part numbers, security/pairing details, plus
  regex patterns for things like `Class::method` or source file names) and
  aborts the release — never publishing the raw text as a fallback — if
  Ollama is unreachable, returns an empty response, breaks the
  Added/Changed/Fixed structure, or the output still contains a denied
  pattern.
...
```bash
~/scripts/ollama-sanitize-changelog.sh --dry-run   # show the diff, write nothing
~/scripts/ollama-sanitize-changelog.sh             # write + commit, confirms before pushing
```
