...
`~/scripts/ai-update-changelog.sh` (a local-Ollama changelog generator
...
`/mint/changelog` is built from the **public, sanitized** `CHANGELOG.public.md`
files of the sibling `../mint/firmware` and `../mint/software` checkouts, not
hand-written and **never** from their internal `CHANGELOG.md`:

```bash
npm run mint-changelog:pull   # copy ../mint/*/CHANGELOG.public.md into scripts/mint-changelog/public/ (gitignored)
...
`scripts/mint-changelog/pull.mjs` never falls back to the internal
`CHANGELOG.md` — if a repo's `CHANGELOG.public.md` is missing, the pull step
fails loudly instead of silently publishing unreviewed internal content.

### Where CHANGELOG.public.md comes from

Each mint repo's own `CHANGELOG.md` (full technical detail — algorithm/filter
names, MCU and sensor part numbers, BLE UUIDs, pairing/security internals,
etc.) is private and never leaves that repo. `CHANGELOG.public.md` is a
separate, marketing-safe rewrite generated by
[`~/scripts/ai-sanitize-changelog.sh`](../../scripts/ai-sanitize-changelog.sh)
(run from **this repo's own `scripts/deploy.sh`** — the only place it runs;
mint repos' `scripts/release.sh` only fill in the private `CHANGELOG.md` via
`ai-update-changelog.sh` and otherwise no longer touch the website or the
public changelog at all) via a local AI model —
same connection mechanism as `ai-commit.sh` / `ai-update-changelog.sh`
(`~/scripts/ai-config.sh`: routes through the `claude` CLI against an
OpenAI-compatible proxy — the same mechanism as the `claude-oi` zsh function
— default model `AI_MODEL=qwen2.5-coder:7b`, overridable per-run with
`-m`/`--model`).

That script:
- keeps the version number, date, and Added/Changed/Fixed section structure
  byte-for-byte (never sent to the model — only the bullet text is rewritten),
- caches already-sanitized releases by a hash of the source entry
  (`.changelog-sanitize-cache.json`, next to `CHANGELOG.public.md`) so a
  published release's public text doesn't drift on a later run — this matters
  more than usual here since the call mechanism isn't guaranteed
  deterministic run-to-run,
- sends every new/changed entry in ONE combined request per run (not one per
  entry) and validates the model's output against each repo's own
  `<repo>/.changelog-deny-list.txt` (technical terms, hardware part numbers,
  security/pairing details, plus regex patterns for things like
  `Class::method` or source file names) — aborting the release, never
  publishing the raw text as a fallback, if the AI call fails, returns an
  empty response, or the output still contains a denied pattern. On a
  failure it prints exactly how to hand-fix that one entry (see "manual
  override" below) instead of a dead-end error.
- lets a stubborn entry (a local model that won't drop a jargon term no
  matter the prompt) be hand-rewritten once via
  `<repo>/.changelog-overrides/<version>.md` — a plain markdown file with
  just that entry's sections/bullets (or, if the leak is in the header's own
  free-text title, a full `## [version] - date — new title` line as the
  override's first line), used instead of asking the model and still
  deny-list-checked.

It only ever **writes files** — it never runs `git add`/`commit`/`push`
itself. Review and commit (`CHANGELOG.public.md` +
`.changelog-sanitize-cache.json`) is up to you.

Run it directly (from inside `../mint/firmware` or `../mint/software`) to
preview a release before tagging:

```bash
~/scripts/ai-sanitize-changelog.sh --dry-run   # show the diff, write/change nothing
~/scripts/ai-sanitize-changelog.sh             # write CHANGELOG.public.md + cache (asks to confirm)
```
